...
img=cv.imread("imaging.seesaw15.jpg")
jerk = Image.fromarray(img)
...
if len(faces_rect) > 0:
    ...
    ncol = math.ceil(len(faces_rect)/2)
    nrow = math.ceil(len(faces_rect)/ncol)
    canvas = Image.new(canvas_control.mode, (int(300*canvas_control.width), int(300*canvas_control.height) ))
    curr_x = 0
    curr_y = 0

#heuristics
    for x,y,w,h in faces_rect:
        face = jerk.crop((x,y,x+w,y+h))
    ...
    #process = Process(target=make_face_dict, args=('test.zip'))
    #process.start()

#extract names
#demo = str(zlist[1])
#pattern = re.compile("output_[0-9]+_[0-9].png")
